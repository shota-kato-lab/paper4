 ...
 *
 * ★Paper4_β_predictor_3 v0.3.2  (tool/6/  2026-05-21)
 *
 * v0.3.1 → v0.3.2 diff (= 著者 chat feedback 4 件 反映):
 *   A. Formula-explainer block restructured into a NESTED <details> element
 *      (default collapsed, centered max-width 720px, /tool/5/ wording verbatim
 *      under the existing "What is this?" disclosure).
 *   B. Zenodo button centered + label switched to English: "Public on Zenodo
 *      (DOI: 10.5281/zenodo.20145297)".
 *   C. Electricity (row 8) physical meaning corrected — was a circular
 *      "deeper hybrid coordination depth (H=6)" statement; now an explicit
 *      6-node relay chain (= the author's H=6 expert identification of the
 *      power-grid coordination layers).
 *   D. Relay-Depth interpretation locked: H = number of boxes 【】 in the
 *      chain diagram (NOT arrow count). Per author chat 2026-05-22 ~00:00 JST
 *      confirmation: "※The number of 【】 is H: The relay-depth.  【A】 → 【B】
 *      = H=2".  All 22 rows rewritten with explicit 【box1】→...→【boxH】
 *      chains so that the number of 【】 visibly equals H.
 *
 * 反省 27-31 strict per (= same as v0.3.0/v0.3.1).
 */
...
function renderDetailPanel(row) {
  ...
  // Relay chain (H-node enumeration) — author confirmed 2026-05-22 ~00:00 JST
  // "The number of 【】 is H".  Each box in the chain is a structural
  // coordination layer per V261 §S-PanelOrigins.
