...
// ============================================================================
// Section 1 — 22-row reference data (hard-coded per Paper 4 V261/V262)
// ============================================================================
//
// Per-row physical_meaning fields are quoted from Paper 4 V264 §S-PanelOrigins
// "β-independent assignment provenance" table (Table tab:beta_independent_assignment,
// L3243-3271).  Verbatim source-text reuse keeps the catalogue aligned with paper
// wording.
//
// For Tier B (mixture) rows the per-H meanings are written from the same V264
// table, with the two H values' interpretations given separately (= "2 つの意味"
// option per author chat 2026-05-21 ~23:40 JST clarification request).
...
const indicators = [
  ...
  // Tier B physical_meaning includes TWO interpretations (= H_low and H_up) +
  // the mixture rationale, per author chat 2026-05-21 ~23:40 JST clarification
  // request ("mixture の場合はその２つの意味...?").
...
function classificationLine(row) {
  const sub = row.substrate;
  return row.network_type + ", " + sub;
}
...
function renderDetailPanel(row) {
  ...
  if (!row.mixture) {
    ...
    // ④ Strengthened wording: mixing weights ex post diagnostic surfaced at H_eff line.
...
function init() {
  ...
  grid.addEventListener('click', function (ev) {
    ...
    // ⑤ Scroll target changed: scroll the clicked button to the top of viewport
    // (= "ボタンの真上が一番上に出る" per author chat feedback 2026-05-21 ~23:40 JST).
