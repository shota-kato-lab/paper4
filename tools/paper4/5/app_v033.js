 ...
 *
 * ★Paper4_β_predictor_3 v0.3.3  (tool/6/  2026-05-22)
 *
 * v0.3.2 → v0.3.3 diff (= 著者 chat feedback 2 件 反映):
 *   E. dformula display extended to TWO lines:
 *      "General formula: β = 1 ± 1 / [H · ln(2H + 1)]"
 *      "This time: β_pred = β+(H=5) = 1 + 1 / [5 · ln(2×5 + 1)] = 1 + 1 / [5 · ln(11)] ≈ 1.083"
 *      i.e., explicit formula + careful substitution per author chat request.
 *      Also applies to balanced rows (Path 1 σ_gc=0 mechanism) and mixture
 *      rows (H_eff substitution).
 *   F. (HTML-side, not in this JS file) nested disclosure content swapped to
 *      /tool/4/ wording (cleaner, no Path A/B specific paragraphs); the
 *      "count 【】" sentence I added in v0.3.2 is removed from there per
 *      author chat request — the 【】-counting explanation lives only in
 *      the per-row detail panels (where the chain is actually shown), not
 *      in the general disclosure.
 *
 * 反省 27-31 strict per (= same as v0.3.0-v0.3.2).
 */
...
// E. Render the dformula block: general formula + this-time substitution.
// Returns an HTML string for the per-row formula display.
function renderFormulaBlock(row) {
  ...
  // Tier A non-balanced (integer H, plus or minus branch)
  const sgn = (row.branch === "minus") ? "−" : "+";
...
function renderDetailPanel(row) {
  ...
  // E. NEW dformula block (general formula + this-time substitution).
